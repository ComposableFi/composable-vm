...
fn find_key_offset(key: &str, data: &str) -> Option<u32> {
    data.find(key)
        .map(|index| index as u32 + key.len() as u32 + 1)
}
    ...
    fn try_into(self) -> Result<CosmosMsg, Self::Error> {
        let m = match self {
            ...
            FlatCosmosMsg::Wasm(FlatWasmMsg::UpdateAdmin {
                contract_addr,
                admin,
            }) => CosmosMsg::Wasm(cosmwasm_std::WasmMsg::UpdateAdmin {
                contract_addr,
                admin,
            }),
...
mod tests {
    ...
    fn test_update_admin() {
        ...
        assert_eq!(
            msg.bindings,
            vec![(65, BindingValue::Register(Register::This))]
        );
    ...
    fn test_bank_send() {
        ...
        assert_eq!(
            msg.bindings,
            vec![(30, BindingValue::Register(Register::This))]
        );
