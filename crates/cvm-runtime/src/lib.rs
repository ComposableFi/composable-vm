    ...
    CurrentNetwork::EncodedCall: Into<Vec<u8>>,
{
    ...
    pub fn transfer(
        ...
        self.instructions.push(Instruction::Transfer {
            to: to.into(),
            assets: assets.into(),
        });
        self
        ...
        FinalNetwork::EncodedCall: Into<Vec<u8>>,
    {
        ...
        builder.instructions.push(Instruction::Spawn {
            ...
            program: f(ProgramBuilder::<SpawningNetwork, Account, Assets>::new(
                tag.into(),
            ))?
            .build(),
        });
        ...
        T: Protocol<CurrentNetwork>,
    {
        protocol
            .serialize()
            .map(|encoded_call| self.call_raw(encoded_call))
    }
...
mod tests {
    ...
    fn can_build() {
        let program = || -> Result<_, ProgramBuildError> {
            Ok(
                ProgramBuilder::<Picasso, (), Funds>::new("Main program".as_bytes().to_vec())
                    .call(DummyProtocol1)?
