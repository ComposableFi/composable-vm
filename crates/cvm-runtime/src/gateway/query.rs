use crate::{
    prelude::*,
    AssetId, NetworkId, exchange::ExchangeItem, ExchangeId,
};
...
pub enum QueryMsg {
    ...
    #[cfg_attr(all(feature = "json-schema", not(target_arch = "wasm32")), returns(GetAssetResponse))]
    ...
    #[cfg_attr(all(feature = "json-schema", not(target_arch = "wasm32")), returns(GetIbcIcs20RouteResponse))]
    GetIbcIcs20Route {
        to_network: NetworkId,
        for_asset: AssetId,
    },

    #[cfg_attr(all(feature = "json-schema", not(target_arch = "wasm32")), returns(GetExchangeResponse))]
