    ...
    fn from(value: CVMPacket<TAbiEncoded, TAccount, TAssets>) -> Self {
        Self {
            ...
            assets: value
                ...
                .0
                .into_iter()
                .map(pb::program::PacketAsset::from)
                .collect(),
        }
    }
}
...
impl From<crate::Amount> for pb::program::Balance {
    fn from(balance: crate::Amount) -> pb::program::Balance {
        let absolute = Some(pb::program::Absolute {
            value: Some(balance.intercept.0.into()),
        });
        let ratio = Some(pb::program::Ratio {
            nominator: balance.slope.0,
        });
        Self { absolute, ratio }
    }
}
