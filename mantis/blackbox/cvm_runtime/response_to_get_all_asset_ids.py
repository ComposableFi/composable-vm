...
#   filename:  response_to_get_all_asset_ids.json
...
class AssetReference11(BaseModel):
    """
    Cosmos SDK native
    """
...
class AssetReference12(BaseModel):
    """
...
class AssetReference15(BaseModel):
    """
...
class ForeignAssetId4(BaseModel):
    """
...
class AssetReference13(BaseModel):
    """
    Definition of an asset native to some chain to operate on. For example for Cosmos CW and EVM chains both CW20 and ERC20 can be actual. So if asset is local or only remote to some chain depends on context of network or connection. this design leads to some dummy matches, but in general unifies code (so that if one have to solve other chain route it can). One consensus(chain) can have assets produced by different protocols(VMs).
    """
...
class AssetReference14(BaseModel):
    """
...
class ForeignAssetId(RootModel[Union[ForeignAssetId3, ForeignAssetId4]]):
    root: Union[ForeignAssetId3, ForeignAssetId4]
...
class ArrayOfAssetItem(RootModel[List[AssetItem]]):
    root: List[AssetItem] = Field(..., title='Array_of_AssetItem')
